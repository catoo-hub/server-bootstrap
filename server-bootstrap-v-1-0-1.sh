...
# ── Allowlist update script (Russian operator ASNs via RADB whois) ───────────
...
SCRIPT
    ...
    log_info "Created /usr/local/bin/update_allowlist.sh"
}
...
# ── Cron: daily 04:20 — update lists → reload ────────────────────────────────
_write_cron() {
    cat > /etc/cron.d/haproxy-lists << 'EOF'
# server-bootstrap: update haproxy IP lists and reload daily at 04:20
20 4 * * * root /usr/local/bin/analyze_logs.sh; /usr/local/bin/update_blocklist.sh; /usr/local/bin/update_allowlist.sh; systemctl reload haproxy
EOF
    log_info "Cron job set: daily 04:20 → update lists + reload haproxy"
}
...
setup_haproxy() {
    ...
    # 5. Fetch initial lists (blocklist fast, allowlist slow ~2-3 min)
    ...
    log_info "Fetching initial allowlist (may take 2-3 min via whois)..."
