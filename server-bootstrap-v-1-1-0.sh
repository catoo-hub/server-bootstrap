...
#
#  Author:  Kitsura VPN
#  Version: 1.1.0 (test build — relay + subscription page on same host)
# ==============================================================================
...
# ── Subscription page (relay co-install, v1.1.0) ─────────────────────────────
# When WITH_SUBPAGE=true and MODE=relay, HAProxy adds SNI-based routing:
#   SNI = SUBSCRIPTION_DOMAIN  → 127.0.0.1:${SUBPAGE_LOCAL_TLS_PORT}  (Caddy → app:3010)
#   any other SNI              → existing blocked/allowed/upstream chain (gate)
# Subpage traffic SKIPS the IP allow/block lists by virtue of being matched first.
WITH_SUBPAGE=false                  # Install subscription page next to relay?
SUBSCRIPTION_DOMAIN=""              # e.g. sub.example.com (must point to RELAY_ADDRESS)
PANEL_URL=""                        # e.g. https://panel.example.com (where the panel lives)
PANEL_API_TOKEN=""                  # API token from panel → Settings → API Tokens
CUSTOM_SUB_PREFIX=""                # optional path prefix for subscription URLs
SUBPAGE_DIR="/opt/web/subscription"
SUBPAGE_CADDY_DIR="/opt/web/caddy"
SUBPAGE_PARENT_DIR="/opt/web"
SUBPAGE_APP_PORT="3010"             # app container port
SUBPAGE_LOCAL_TLS_PORT="8443"       # Caddy local TLS bind (HAProxy backend target)
...
declare -A CHANGELOG=(
    ...
    ["1.1.0"]="Relay mode: optional co-install of subscription page on the same server. HAProxy gains SNI-based split: traffic for SUBSCRIPTION_DOMAIN bypasses blocked.lst/allowed.lst and goes to local Caddy (127.0.0.1:8443) → subscription container (:3010); all other SNIs keep the existing blocked/allowed/upstream chain to gate. Caddy auto-issues TLS via Let's Encrypt HTTP-01 on port 80 (HAProxy doesn't bind :80). New CLI flags: --with-subpage, --subscription-domain, --panel-url, --api-token, --sub-prefix."
)
...
if [[ -t 1 ]] && command -v tput &>/dev/null && tput colors &>/dev/null && [[ "$(tput colors)" -ge 8 ]]; then
    ...
    YELLOW='[1;33m'; BLUE='[0;34m'
    CYAN='[0;36m';   MAGENTA='[0;35m'
    WHITE='[1;37m';  GRAY='[0;37m'
    BOLD='[1m';      RESET='[0m'
else
...
# Fancy header
print_header() {
    local title="SERVER BOOTSTRAP  ·  ${SCRIPT_VERSION}"
    local subtitle="Debian 12+ / Ubuntu 22.04+  |  root only"
...
setup_ufw() {
    ...
    # OpenVZ/LXC containers often lack iptables — UFW will fail silently
    if [[ "${IS_CONTAINER:-false}" == true ]]; then
        log_warn "Container virtualization (${VIRT_TYPE}) detected — UFW/iptables may not work"
        log_warn "Skipping UFW. Configure firewall rules at the host node or hosting panel."
    ...
    case "$mode" in
        ...
        relay)
            ...
            if [[ "${WITH_SUBPAGE:-false}" == true ]]; then
                # Caddy needs :80 for Let's Encrypt HTTP-01 challenge.
                # HAProxy in relay mode does NOT bind :80 — no conflict.
                ufw allow 80/tcp comment 'Caddy ACME (subpage)' &>/dev/null
                log_info "UFW: opened 80/tcp for Caddy ACME (subpage co-install)"
            fi
            ;;
        monitoring)
...
# ── Ask / validate Subscription page parameters (relay co-install) ──────────
_ask_subpage_params() {
    # SUBSCRIPTION_DOMAIN — must be a valid hostname pointing to RELAY_ADDRESS
    ...
    # PANEL_URL — full https://panel.example.com URL
    if [[ -z "${PANEL_URL:-}" ]]; then
        if [[ "$NON_INTERACTIVE" == true ]]; then
    ...
    # PANEL_API_TOKEN — generated in panel: Settings → API Tokens
    ...
    # CUSTOM_SUB_PREFIX — optional, default empty
    if [[ "$NON_INTERACTIVE" == false && -z "${CUSTOM_SUB_PREFIX:-}" ]]; then
        read -rp "  Custom subscription path prefix (empty = none): " CUSTOM_SUB_PREFIX
    fi

    log_info "SUBPAGE: ${SUBSCRIPTION_DOMAIN} → 127.0.0.1:${SUBPAGE_LOCAL_TLS_PORT} (Caddy → :${SUBPAGE_APP_PORT})"
...
_write_haproxy_cfg() {
    ...
    # Optional SNI route for the subscription page (matched BEFORE IP filters,
    # so subpage clients are NOT subject to blocked.lst / allowed.lst).
...
backend bk_subpage
    server caddy 127.0.0.1:${SUBPAGE_LOCAL_TLS_PORT}
"
    fi
...
EOF
    ...
    if [[ "${WITH_SUBPAGE:-false}" == true ]]; then
        log_info "HAProxy relay config written (SNI split: ${SUBSCRIPTION_DOMAIN} → local Caddy, others → ${GATE_ADDRESS}:${GATE_PORT})"
    else
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 15a · SUBSCRIPTION PAGE (relay co-install, v1.1.0)
#
# Layout on disk:
#   ${SUBPAGE_DIR}/
#     ├── docker-compose.yml      — app container (binds 127.0.0.1:3010)
#     └── .env                     — APP_PORT + panel URL + API token (mode 600)
#   ${SUBPAGE_CADDY_DIR}/
#     ├── Caddyfile               — reverse-proxies https://${SUBSCRIPTION_DOMAIN} → :3010
#     └── docker-compose.yml      — Caddy 2.9 (binds 0.0.0.0:80, 127.0.0.1:8443)
#
# Networking:
#   :80    Caddy direct (ACME HTTP-01)
#   :443   HAProxy → SNI=${SUBSCRIPTION_DOMAIN} → 127.0.0.1:8443 (Caddy) → :3010 (app)
# ─────────────────────────────────────────────────────────────────────────────

setup_subpage() {
    log_step "Installing subscription page (relay co-install)"
    ...
    if [[ "$DRY_RUN" == true ]]; then
        ...
        log_dry "Would create docker network '${SUBPAGE_NETWORK_NAME}' and bring up both stacks"
        log_dry "Caddy would request Let's Encrypt cert for ${SUBSCRIPTION_DOMAIN:-?} via HTTP-01 (port 80)"
        STEP_STATUS["subpage"]="DRY"
        return 0
    fi

    # 1. Collect parameters (idempotent — skips already-set values)
    _ask_subpage_params
...
YAML
    ...
    # 5. .env (chmod 600 — contains API token).
    #    NOTE: env variable names below are dictated by the upstream image and
    #    cannot be renamed. File mode 600 keeps them out of unprivileged eyes.
...
EOF
    ...
    # 6. Caddyfile — local TLS bind on 127.0.0.1:${SUBPAGE_LOCAL_TLS_PORT}.
    #    HAProxy sends SNI=${SUBSCRIPTION_DOMAIN} traffic here (TCP passthrough).
    #    Caddy ACME HTTP-01 challenges arrive on :80 directly (HAProxy doesn't bind 80).
    backup_file "${SUBPAGE_CADDY_DIR}/Caddyfile" 2>/dev/null || true
    cat > "${SUBPAGE_CADDY_DIR}/Caddyfile" <<EOF
{
    # ACME HTTP-01 challenge port (must match Caddy's :80 bind below)
    http_port 80
    # Local TLS port — HAProxy routes SNI=${SUBSCRIPTION_DOMAIN} → 127.0.0.1:${SUBPAGE_LOCAL_TLS_PORT}
...
EOF
    ...
    # 7. Caddy docker-compose.yml — binds :80 (host) + :${SUBPAGE_LOCAL_TLS_PORT} (loopback only)
...
YAML
    ...
    # 8. Bring up app first, then Caddy
    ...
    echo ""
    log_info "Useful commands:"
    echo -e "    ${CYAN}# App logs${RESET}"
    echo    "    docker compose -f ${SUBPAGE_DIR}/docker-compose.yml logs -f"
    echo -e "    ${CYAN}# Caddy logs (cert acquisition)${RESET}"
    echo    "    docker compose -f ${SUBPAGE_CADDY_DIR}/docker-compose.yml logs -f"
    echo -e "    ${CYAN}# Verify TLS chain via HAProxy${RESET}"
    echo    "    curl -vI https://${SUBSCRIPTION_DOMAIN}/"
}
...
uninstall_subpage() {
    ...
    # Remove the empty parent if nothing else lives there
...
run_uninstall() {
    ...
    for step in haproxy haproxy_gate subpage monitoring fail2ban ufw remnanode selfsteal mobile443 docker sysctl_network; do
    ...
    if [[ ${#components[@]} -eq 0 ]]; then
        ...
        components=(haproxy haproxy_gate subpage monitoring fail2ban ufw remnanode selfsteal mobile443 docker sysctl_network)
...
run_relay() {
    ...
    # Ask early — affects UFW (port 80) and HAProxy config (SNI route).
    # In non-interactive mode, WITH_SUBPAGE comes from --with-subpage flag.
    if [[ "$NON_INTERACTIVE" == false && "${WITH_SUBPAGE:-false}" != true ]]; then
        ...
        if [[ "${_sp_ans,,}" == "y" ]]; then
            WITH_SUBPAGE=true
            _ask_subpage_params   # collect params upfront so UFW/HAProxy steps see them
        fi
    elif [[ "${WITH_SUBPAGE:-false}" == true && -z "${SUBSCRIPTION_DOMAIN:-}" ]]; then
        # Non-interactive: validate required params now, fail fast
        _ask_subpage_params
    fi
...
show_menu() {
    ...
    while true; do
        ...
        echo -e "  ${CYAN}4)${RESET} relay   — Relay/Node Relay mode (base + haproxy + optional subscription-page)"
...
${BOLD}Options:${RESET}
  ...
  --with-subpage             [relay] Co-install subscription page on this server
  --subscription-domain <d>  [relay+subpage] Subscription page domain (e.g. sub.example.com)
  --panel-url <url>          [relay+subpage] Panel URL (https://panel.example.com)
  --api-token <token>        [relay+subpage] Panel API token (Settings → API Tokens)
  --sub-prefix <prefix>      [relay+subpage] Optional CUSTOM_SUB_PREFIX for subscription URLs
...
${BOLD}Examples:${RESET}
  ...
  # Relay + subscription-page on the same server (v1.1.0)
